...
class OD():
  def __init__(self,model_path,num_class):
      ...
      self.feature = GraphSeg(inchannel = 3,hidden = 64,num_classes = 2)

      model = torch.load(model_path,map_location = self.device)
      #model = model['featuremodel']

      self.interp = nn.Upsample(size = (self.input_shape [0],self.input_shape [1]),mode = 'bilinear',align_corners = True)
