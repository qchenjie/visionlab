...
from nets.GCN import Grapher, act_layer
...
class GEDNet(nn.Module):
    def __init__(self, n_channels, num_classes, bilinear = True):
        ...
        item = 0
        HW_new = 224 // 4 * 224 // 4
        self.backbone1, item = self.basic_backbone(channels[ 0 ], blocks[ 0 ], dpr, num_knn, 4, max_dilation, item,
                                                   HW_new)
        self.downsample1 = Down(channels[ 0 ], channels[ 1 ])
        HW_new = HW_new // 4
        self.backbone2, item = self.basic_backbone(channels[ 1 ], blocks[ 1 ], dpr, num_knn, 2, max_dilation, item,
                                                   HW_new)
        self.downsample2 = Down(channels[ 1 ], channels[ 2 ])
        HW_new = HW_new // 4
        self.backbone3, item = self.basic_backbone(channels[ 2 ], blocks[ 2 ], dpr, num_knn, 1, max_dilation, item,
                                                   HW_new)
        self.downsample3 = Down(channels[ 2 ], channels[ 3 ])
        HW_new = HW_new // 4
        self.backbone4, item = self.basic_backbone(channels[ 3 ], blocks[ 3 ], dpr, num_knn, 1, max_dilation, item,
                                                   HW_new)
        ...
        self.conv_backbone1, item = self.basic_backbone(channels[ 0 ], 1, dpr_deconv, num_knn, 4, max_dilation, item,
                                                        HW_new_1)
        HW_new_1 = HW_new_1 // 4
        self.conv_backbone2, item = self.basic_backbone(channels[ 1 ], 1, dpr_deconv, num_knn, 2, max_dilation, item,
                                                        HW_new_1)
        HW_new_1 = HW_new_1 // 4
        self.conv_backbone3, item = self.basic_backbone(channels[ 2 ], 1, dpr_deconv, num_knn, 1, max_dilation, item,
                                                        HW_new_1)
        HW_new_1 = HW_new_1 // 4
        self.conv_backbone4, item = self.basic_backbone(channels[ 3 ], 1, dpr_deconv, num_knn, 1, max_dilation, item,
                                                        HW_new_1)
...
if __name__ == '__main__':
    ...
    model = GEDNet(n_channels = 3,num_classes = 2)
