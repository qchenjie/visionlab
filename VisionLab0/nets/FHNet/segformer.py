...
class Scale_reduce(nn.Module):
    def __init__(self, dim, reduction_ratio, img_size):
        super().__init__()
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if (len(self.reduction_ratio) == 4):
            ...
            tem0 = x[ :, :c1, : ].reshape(B, chan1, chan1, C).permute(0, 3, 1, 2)
            # print(tem0.shape,x[:,c1:c2,:].shape)
            tem1 = x[ :, c1:c2, : ].reshape(B, chan2, chan2, C * 2).permute(0, 3, 1, 2)
            tem2 = x[ :, c2:c3, : ].reshape(B, chan3, chan3, C * 5).permute(0, 3, 1, 2)
            tem3 = x[ :, c3:c4, : ]
...
class M_EfficientSelfAtten(nn.Module):
    def __init__(self, dim, head, reduction_ratio, img_size):
        super().__init__()
        self.head = head
        ...
        if reduction_ratio is not None:
            self.scale_reduce = Scale_reduce(dim, reduction_ratio, img_size)
...
#   TODO？ 替换这一块
#---------------------------------------------------------------#
class DCPool(nn.Module):
    def __init__(self,
                 patch, #TODO:? downsample times
                 inc, # inc
                 outc, # outc
                 ksize,
                 stride,
                 padding,
                 mode = 'DC'):
        super(DCPool, self).__init__()


        self.mode = mode #select mode
        self.maxpool = nn.Sequential(nn.MaxPool2d(kernel_size = ksize,
                                                     stride = stride,
                                                     padding = padding
                                                     ),
        ...
        self.avgpool = nn.Sequential(nn.AvgPool2d(kernel_size = ksize,
                                                     stride = stride,
                                                     padding = padding
                                                     ),
        ...
        self.xpool = nn.Sequential(
                    nn.AdaptiveAvgPool2d([patch,1]
                                 ),
                    nn.ReLU(inplace = True)
        )

        self.ypool = nn.Sequential(
            nn.AdaptiveAvgPool2d([ 1, patch ]
                                 ),
            nn.ReLU(inplace = True)
        )
        ...
        if self.mode == 'LC':
            self.project = nn.Conv2d(in_channels = 3 * inc ,
                                     out_channels = outc,
                                     kernel_size = 1)
            self.conv = nn.Conv2d(in_channels = outc,
                                     out_channels = outc,
                                     kernel_size = 1,
                                     stride = 1)
            self.righconv = nn.Conv2d(in_channels = inc,
                                     out_channels = outc,
                                     kernel_size = ksize,
                                     stride = stride,
                                     padding = padding)
    ...
    def forward(self,x):
      if self.mode == 'DC':
        ...
        x1 = self.xpool(x)
        y = self.ypool(x)
        out = self.layer(x1 + y) # TODO add boardcast

        # TODO maxpool
        x_max = self.maxpool(x) + out

        x_max = (x_max + x)
        # TODO avgpool
        x_avg = self.avgpool(x_max) + out
        ...
        x2out = self.righconv(x)
        out = self.project(torch.cat((outm,outa,outgrid),dim = 1)) + x2out
        out = self.conv(out)

        return out
      else:
          raise NotImplementedError('NO exist mode! Please check mode whether use correct!!!')
...
class OverlapPatchEmbeddings(nn.Module):
    def __init__(self, img_size = 224, patch_size = 7, stride = 4, padding = 1, in_ch = 3, dim = 768):
        ...
        ds = (img_size - patch_size + 2 * padding) // stride + 1
        # self.out = nn.Sequential(DCPool(patch = ds,inc = in_ch, outc = dim, ksize = patch_size,
        #                                 stride = stride, padding = padding, mode = 'LC'),
        #                          nn.GELU(),
        #
        #                          nn.Dropout()
        #                          )
