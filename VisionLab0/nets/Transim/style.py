...
from nets.Transim.datasets import UnetDataset, collate_seg
...
class StyleTransfer(nn.Module):
    ...
    def __init__(self, scale_factor = 8,fs = 128):
        ...
        self.layer1 = nn.Sequential(nn.Linear(3 * fs * fs, 1024),
                                    nn.Dropout(0.5),
...
if __name__ == '__main__':
    ...
    source_file = '/root/data1/Gasking_Segmentation/kvasir_seg_train.txt'
    target_file = '/root/data1/Gasking_Segmentation/kvasir_seg_train.txt'
    
    traindata = UnetDataset(source_file = source_file,
                            target_file = target_file)
    
    trainloader = DataLoader(batch_size = 2, dataset = traindata, collate_fn = collate_seg)
    
    device = torch.device('cuda') if torch.cuda.is_available() else torch.device('cpu')
    
    model = StyleTransfer(fs = 256).to(device)
    
    model.load_state_dict(torch.load(r'/root/data1/Gasking_Segmentation/model_last_10.pth'))
    ...
    for epoch in range(init_iter, max_iter):
        ...
        for batch, (images, cdfs) in enumerate(trainloader):
            ...
            loss.backward()
            print(
                f'INFO:{epoch}/{max_iter}==============ssim:{ssim(cdfs.detach().cpu().numpy(), recon.detach().cpu().numpy())}======pnsr:{pnsr(cdfs.detach().cpu().numpy(), recon.detach().cpu().numpy())}===loss:{loss.item()}================')
        ...
        if ((epoch + 1) % 10 ) == 0:
            ...
            torch.save(model.state_dict(), f'model_last_{epoch}.pth')
