...
from nets.SETR.transformer_model import TransModel2d, TransConfig
import math
...
class Decoder2D(nn.Module):
    def __init__(self, in_channels, out_channels, features = [ 512, 256, 128, 64 ]):
        ...
        self.decoder_5 = nn.Sequential(
            nn.Conv2d(features[ 3 ], features[ -1 ], 3, padding = 1),
            nn.BatchNorm2d(features[ -1 ]),
            nn.ReLU(inplace = True),
            nn.Upsample(scale_factor = 2, mode = "bilinear", align_corners = True)
        )
...
if __name__ == '__main__':
    #from nets.SETR.transformer_seg import SETRModel, Vit
